%{
    ...
    void generateCode(symtab_t *, char[]);
    void postOrder(symtab_t *, headOrRoot *, node_t *, char[]);
    int postOrderUtil(symtab_t *, headOrRoot *, node_t *, char *, int *, char[]);
...
int main(int argc, char** argv)
{
    ...
    static char Instructions[10000];
    generateCode(global, Instructions);
...
void generateCode(symtab_t *global, char Instructions[])
{
    headOrRoot *stmt = global->MChead;
    ...
    {
        if(stmt->root != NULL)
            postOrder(global, stmt, stmt->root, Instructions);
    ...
    if(global->child != NULL)
        generateCode(global->child, Instructions);

    if(global->next != NULL)
        generateCode(global->next, Instructions);

    return;
}

void postOrder(symtab_t *table, headOrRoot *stmt, node_t *root, char Instructions[])
{
    static int i = -1;
    char type[10];
    ...
    postOrderUtil(table, stmt, root->right, type, &i, Instructions);
    printf("%s = t%d\n", root->left->op, i);

    // Instruction for Assignment statement.
    char ins[35];
    sprintf(ins, "ST %s R0\n\n", root->left->op);
    strcat(Instructions, ins);
}

int postOrderUtil(symtab_t *table, headOrRoot *stmt, node_t *root, char type[], int *i, char Instructions[])
{
    if(root->left->left == NULL && root->left->right == NULL && root->right->left == NULL && root->right->right == NULL)
    {
        char type1[10], type2[10];
        int leftType = isNumber(root->left->op);
        int rightType = isNumber(root->right->op);

        // Finding type of left operand.
        if(leftType == 0)
        ...
        // Instruction for left operand loading.
        char ins[35];
        if(leftType == 0)
            sprintf(ins, "LD R0 %s\n", root->left->op);
        else
        ...
        // Instruction for right operand loading.
        if(rightType == 0)
            sprintf(ins, "LD R1 %s\n", root->right->op);
        else
            sprintf(ins, "LI R1 %s\n", root->right->op);
        ...
        // Instruction for operation involved.
        if(strcmp(root->op, "+") == 0)
            sprintf(ins, "ADD R0 R0 R1\n");
        else if(strcmp(root->op, "-") == 0)
            sprintf(ins, "SUB R0 R0 R1\n");
        else if(strcmp(root->op, "*") == 0)
            sprintf(ins, "MULT R0 R0 R1\n");
        else if(strcmp(root->op, "/") == 0)
            sprintf(ins, "DIV R0 R0 R1\n");
        strcat(Instructions, ins);

        // Instruction for storing back.
        sprintf(ins, "ST t%d R0\n\n", *i);
        strcat(Instructions, ins);
    ...
    else if(root->left->left == NULL && root->left->right == NULL && root->right->left != NULL && root->right->right != NULL)
    {
        ...
        // Finding type of the left operand.
        if(leftType == 0)
            strcpy(type1, varCheck(table, stmt, root->left->op));
        else if(leftType == 1)
            strcpy(type1, "int");
        ...
        // Recursive Code Generation on right subtree.
        int index2 = postOrderUtil(table, stmt, root->right, type, i, Instructions);
        ...
        // Instruction for right operand(temporary variable in this case) loading.
        sprintf(ins, "LD R1 t%d\n", index2);
        strcat(Instructions, ins);
    ...
    else if(root->left->left != NULL && root->left->right != NULL && root->right->left == NULL && root->right->right == NULL)
    {
        ...
        // Finding type of the right operand.
        if(rightType == 0)
            strcpy(type2, varCheck(table, stmt, root->right->op));
        else if(rightType == 1)
            strcpy(type2, "int");
        ...
        // Recursive Code Generation on left subtree.
        int index1 = postOrderUtil(table, stmt, root->left, type, i, Instructions);
        ...
        if(strcmp(type2, type) == 0)
         ...
         // Instruction for left operand(temporary variable in this case) loading.
        char ins[35];
        sprintf(ins, "LD R0 t%d\n", index1);
        strcat(Instructions, ins);
